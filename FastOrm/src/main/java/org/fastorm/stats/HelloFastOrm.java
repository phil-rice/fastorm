...
public class HelloFastOrm {
	...
	public static void main(String[] args) throws Exception {
		DataSource dataSource = new XmlBeanFactory(new ClassPathResource("MySqlDataSource.xml", HelloFastOrm.class)).getBean(DataSource.class);
		IEntityDefn defn = IEntityDefn.Utils.parse(new TempTableMakerFactory(), new ClassPathResource("sample.xml", HelloFastOrm.class));
		System.out.println("Starting");
		while (true) {
			int count = 0;
//			IFastOrmContainer fastOrm = IFastOrm.Utils.mySqlSingleThreaded(defn, dataSource).withDataSize(100).getContainer();
			IFastOrmContainer fastOrm = IFastOrm.Utils.mySqlSingleThreaded(defn, dataSource).withDataSize(100).withSqlLogger(new SysOutSqlLogger()).getContainer();
			IEntityReader<ISimpleMap<String, Object>> reader = fastOrm.makeReader();
			long forStartTime = System.currentTimeMillis();
			for (@SuppressWarnings("unused") ISimpleMap<String, Object> item : reader)
				count++;
